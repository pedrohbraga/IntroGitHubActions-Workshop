...
.footnote[.tiny[Metaphor from fields in organizational development; _e.g._ [Lee & Choi, 2003](doi.org/10.1080/07421222.2003.11045756)]]
...
.center[
Data + Code + Metadata → VCS → Analysis → Report/Manuscript → Submission
]
...
CI decreases integration problems, ensures
rapid feedback, increases software quality, and improves developer productivity (Vasilescu _et al_., 2015; Stahl _et al_., 2019; Rossi _et al_. 2016; Hilton _et al_. 2016).
...
YAML (originally, **y**et **a**nother **m**arkup **l**anguage) is a human-readable programming language. It is important to remember that _indentation matters_ in YAML!
...
Jobs can run sequentially or in parallel. Here, we specify that our second job `needs: presentation-en-job` to make the jobs run sequentially. More on this later!
...
.pull-left[
Each job is run in a specific __working directory__.
...
You could set API keys as environment variables in a workflow that routinely pulls a dataset using an API, for example. 
...
__Artifacts__ are temporary files (or several files) generated during a workflow run.

You can __share__ artifacts between jobs within a workflow. 

For example, you can build an output, save the artifact, and pass it into a job that tests it.

GitHub stores workflow artifacts for __90 days__ by default.
